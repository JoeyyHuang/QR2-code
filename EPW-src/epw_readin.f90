  ...
  USE epwcom,        ONLY : delta_smear, nsmear, dis_win_min, dis_win_max, wannierize, &
                            ...
                            prtRaman, Raman_type, lhoph, reson_lim, reson_thr,         &
                            Elaser, Egamma, polar, filraman, ei, es, lBE, temphon,     &
                            ltensor, qtensor_start, qtensor_end, prtdipole, Cq
  ...
  NAMELIST / inputepw / &
       ...
       ! Added for Raman calculations, by Jianqi Huang
       prtRaman, Raman_type, lhoph, reson_lim, reson_thr, Elaser, Egamma,      &
       polar, filraman, ei, es, Cq, ltensor, qtensor_start, qtensor_end,       &
       prtdipole, lBE, temphon
  ! --------------------------------------------------------------------------------
  !
  ! amass    : atomic masses
  ...
  !
  ! Added for Raman calculations, by Jianqi Huang
  ! prtRaman      : if .TRUE., print Raman intensity
  ! Raman_type    : if 'single' then computes single-resonance Raman; 'double' computes double-resonance Raman;
  !                    'defect' computes defect-induced double-resonance Raman
  ! lhoph         : if .TRUE., consider the hole-phonon coupling
  ! reson_lim     : if .TRUE., take a limitation on the energy level seperation in resonant transition
  ! reson_thr     : if 'reson_lim' = .TRUE., then set the threshold value of limitation
  ! Elaser        : the incident laser energy (eV)
  ! Egamma        : electronic energy broadening, relevant to the electron life (eV)
  ! polar         : if 'custom', calculate the Raman in given polarization, if 'all', output all possible polarization
  ! filraman      : if polar = 'custom', the file name where the Raman intensity is stored
  ! ei            : if polar = 'custom', incident light polarization
  ! es            : if polar = 'custom', scattered light polarization
  ! Cq            : The coefficients of each power of q in formula of q-dependent defect-electron scattering
  ! lBE           : if .true., consider the Bose-Einstein distribution of phonons
  ! temphon       : if lBE = .true., the temperature at which phonons BE distribution is considered (K)
  ! ltensor       : if .TRUE., print Raman tensor
  ! qtensor_start : if ltensor = .true., this indicates the first q of which the Raman tensor is printed
  ! qtensor_end   : if ltensor = .true., this indicates the last q of which the Raman tensor is printed
  ! prtdipole     : if .TRUE., print the interpolated dipole matrix elements
  !
...
101   CALL errore('epw_readin', 'opening file ' // filqf, ABS(ios))
  ...
  ! Added for Raman calculations, by Jianqi Huang
  ! from eV to Ryd
  Elaser = Elaser / ryd2ev
  Egamma = Egamma / ryd2ev
  reson_thr = reson_thr / ryd2ev
  ! from K to Ryd
  temphon = temphon * kelvin2eV / ryd2ev
  !
