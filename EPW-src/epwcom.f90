  ...
  LOGICAL :: prtdipole
  !! if .TRUE. print the interpolated dipole matrix elements
  LOGICAL :: prtRaman
  !! if .TRUE. print the Raman intensity
  LOGICAL :: lhoph
  !! if .TRUE. consider the hole-phonon coupling
  LOGICAL :: reson_lim
  !! if .TRUE. take a limitation on the energy level seperation in resonant transition
  LOGICAL :: ltensor
  !! if .TRUE. print Raman tensor
  LOGICAL :: lBE
  !! if .TRUE. consider the Bose-Einstein distribution of phonons
  INTEGER :: qtensor_start
  !! if ltensor = .true., this indicates the first q of which the Raman tensor is printed
  INTEGER :: qtensor_end
  !! if ltensor = .true., this indicates the last q of which the Raman tensor is printed
  REAL(KIND = DP) :: temphon
  !! if 'lBE' = .TRUE. then set the temperature (K) at which phonons BE distribution is considered
  REAL(KIND = DP) :: reson_thr
  !! if 'reson_lim' = .TRUE. then set the threshold value of limitation
  REAL(KIND = DP) :: Elaser
  !! the incident laser energy (eV)
  REAL(KIND = DP) :: Egamma
  !! electronic energy broadening, relevant to the electron life (eV)
  REAL(KIND = DP) :: Cq(7)
  !! The coefficients of each power of q in formula of q-dependent defect-electron scattering
  COMPLEX(KIND = DP) :: ei(3)
  !! incident light polarization
  COMPLEX(KIND = DP) :: es(3)
  !! scattered light polarization
  CHARACTER(LEN = 6) :: polar
  !! if 'custom', calculate the Raman in given polarization, if 'all', output all possible polarization
  CHARACTER(LEN = 50) :: filraman
  !! iif polar = 'custom', the file name where the Raman intensity is stored
  CHARACTER(LEN = 6) :: Raman_type
  !! 'single' computes single-resonance Raman; 'double' computes double-resonance Raman; 
