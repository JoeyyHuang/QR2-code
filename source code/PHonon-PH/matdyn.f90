...
PROGRAM matdyn
  ...
  !
  INTEGER:: nline  ! Added for Raman calculations, by Jianqi Huang
  INTEGER:: nax, nax_blk
  ...
  LOGICAL :: readtau, la2F, xmlifc, lo_to_split, na_ifc, fd, nosym, loto_2d, &
             huang, read_lr, write_frc, lphsort  ! Added for Raman calculations, by Jianqi Huang
  !
  ...
  NAMELIST /input/ flfrc, amass, asr, flfrq, flvec, fleig, at, dos,  &
       ...
       &           lphsort  ! Added for Raman calculations, by Jianqi Huang
  !
  CALL mp_startup()
  ...
  IF (ionode) CALL input_from_file ( )
     ...
     read_lr = .false.
     write_frc = .false.
     lphsort = .false.  ! Added for Raman calculations, by Jianqi Huang
     !
     !
...
20                  IF (ios /=0) CALL errore('matdyn',&
     ...
     DO n=1, nq
        ...
        IF (eigen_similarity) THEN
           ...
           IF (lphsort .AND. n == nq) THEN
             OPEN (UNIT = 10, FILE = 'phonon_sort', STATUS = 'old', IOSTAT = ios)
             IF (ios /= 0) THEN
               OPEN (UNIT = 10, FILE = 'phonon_sort', STATUS = 'new')
               WRITE (10, '(a)') '#     iq     qx     qy     qz     ordering_branch     ordering_frequency'
             ENDIF
             nline = 0
             DO WHILE (.true.)
               READ(10, *, IOSTAT = ios)
             ...
             ENDDO
             BACKSPACE(10)
             WRITE(10, '(i5, 3f10.6)') nline / 3 + 1, q(:,n)
           ENDIF
           !
           ...
           ELSE
              ...
              DO na=1,3*nat
                 ...
                 ! Added for Raman calculations, by Jianqi Huang
                 IF (lphsort .AND. n == nq) WRITE(10, '(5x, i3)', ADVANCE = 'no') location(1)
                 !
              ...
              IF (lphsort .AND. n == nq) THEN
                WRITE (10, *)
                DO na=1, 3*nat
                  WRITE(10, '(2x, f10.4)', ADVANCE = 'no') SQRT(ABS(tmp_w2(na))) * RY_TO_CMM1
                ENDDO
                WRITE (10, *)
                CLOSE (10)
              ENDIF
