program phonon_sort
  ...
  logical :: loto_2d, read_lr
  character(len=256) :: asr
  character(len=256) :: flfrc, matdyn_path, command
  namelist /q_grid/ nq1, nq2, nq3, dql, density, flfrc, asr, loto_2d, read_lr, matdyn_path
  !
  !default values for the necessary variables 
  nq1 = 60         ! q numbers for each of three axes in the fine grid
  nq2 = 60
  nq3 = 1         
  dql = 0.005      ! starting distance away from Gamma point
  density = 500    ! number density of q-points for catching the branch continuity
  flfrc = 'ifc'
  asr = 'simple'
  loto_2d = .false.
  read_lr = .false.
  matdyn_path = '/path/to/matdyn.x'
  !
  ! read variables from input file "phonon_sort.in"
  read (*, nml = q_grid)
  !
  command = 'mpirun -np 1 ' // trim(adjustl(matdyn_path)) // ' <matdyn.in> matdyn.out'
  !
  open (10, file=trim(adjustl(flfrc)), status='old')
...
100     q_crys(1) = dot_product(a1,q_cart)
        ...
        write (10, '(a)') '&input'
        write (10, '(a)') "flfrc = '"//trim(adjustl(flfrc))//"'"
        write (10, '(a)') "asr = '"//trim(adjustl(asr))//"'"
        if (loto_2d) write (10, '(a)') 'loto_2d = .true.'
        if (read_lr) write (10, '(a)') 'read_lr = .true.'
