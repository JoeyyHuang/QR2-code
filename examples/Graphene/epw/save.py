...
for iqpt in np.arange(1,nqpt+1):
  ...
  if (iqpt == 1):
    ...
    # if PAW
    # os.system('cp ../tmp/_ph0/'+prefix+'.dvscf_paw1 save/'+prefix+'.dvscf_paw_q'+label)
  else:
    os.system('cp ../tmp/_ph0/'+prefix+'.q_'+str(iqpt)+'/'+prefix+'.dvscf1 save/'+prefix+'.dvscf_q'+label)
    # if PAW
    # os.system('cp ../tmp/_ph0/'+prefix+'.q_'+str(iqpt)+'/'+prefix+'.dvscf_paw1 save/'+prefix+'.dvscf_paw_q'+label)
