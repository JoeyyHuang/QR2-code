...
!-----------------------------------------------------------------------
  ...
  LOGICAL :: lRay_sca
  !! if .true., consider the Rayleigh scattering. It is recomended to set .false. 
  !! in order to undermine the strong Raman intensity in the vicinity of 0 cm-11
  REAL(KIND = 8) :: Ray_thr
  !! if lRay_sca = .false., this set the radius threshold centered at 0 cm-1, 
  !! within which the Raman intensity is undermined
  LOGICAL :: alive
  ...
  IF (my_id == 0) THEN
    ...
    READ(*, NML = ANALYSIS, IOSTAT=ierr)
    IF (ierr /= 0) lRaman_modes = .false.
    !
